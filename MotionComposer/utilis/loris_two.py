...
class LORIS(nn.Module):
    def __init__(self, config):
        ...
        self.refineNet = AudioDiffusionConditional(
                in_channels=2, 
                channels=256, 
                patch_blocks=1, 
                patch_factor=32,
    ...
    def sample(self, input):
        ...
        content = input['music']
        refer_content = input['refer_music']
        if self.use_genre:
        ...
        refer_content = refer_content.expand(content.shape[0], 2, content.shape[2])
        refer_content = refer_content[:, :, :2 ** self.diffusion_length]
        # print(cond_rhy_peak)
        # exit()
        ...
        noise = noise+samples
        noise = torch.cat((noise,refer_content),dim=0)
        refined_samples = self.refineNet.sample(
                    noise,
                    num_steps=self.diffusion_step,
                    embedding=cond_emb,
                    embedding_scale=self.embedding_scale, # Classifier-free guidance scale
    ...
    def forward(self, input):
        ...
        if self.autoencoder_type == 'diffusion':
            ...
            # print('debug content',content[:, :, :2 ** self.diffusion_length])
            if self.use_genre:
                loss_1,generated = self.cond_diffusion(content[:, :, :2 ** self.diffusion_length], embedding=cond_emb, rhythm=cond_rhy_peak) 
               
                # print('debug content',content.shape,generated.shape)
          
                # print('debug content',content.shape,generated.shape)
                # exit()
                refer_content = refer_content.clone()+generated.clone()
                content = torch.cat((content[:, :, :2 ** self.diffusion_length],refer_content),dim=0)
                loss_2,_= self.refineNet(content[:, :, :2 ** self.diffusion_length], embedding=cond_emb, rhythm=cond_rhy_peak, genre=cond_genre)
                loss = loss_1 + loss_2
