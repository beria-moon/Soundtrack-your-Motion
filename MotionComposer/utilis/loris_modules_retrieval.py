...
class LORIS(nn.Module):
    ...
    def forward(self, input):
        ...
        refer_content = refer_content.expand(content.shape[0], 2, content.shape[2])
        # c= refer_content[:, :, :2 ** self.diffusion_length]
        content = torch.cat((content,refer_content),dim=0)

        # print('debug refer', refer_content.shape,content.shape)
        if self.autoencoder_type == 'diffusion':
            loss = self.diffusion(content[:, :, :2 ** self.diffusion_length])
            
        elif self.autoencoder_type == 'cond_diffusion':
            if self.use_genre:
                # print('debug',content[:, :, :2 ** self.diffusion_length].shape)
