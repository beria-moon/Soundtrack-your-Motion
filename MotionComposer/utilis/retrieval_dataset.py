...
class S25Dataset(Dataset):
    def __init__(self, audio_files,retrieval_audio_files, motion_files, video_files, genre_label, augment, config):
        self.config = config
        self.sampling_rate = config['sample_rate']
        self.segment_length = self.sampling_rate * config['segment_length']
        self.retrieval_audio_files = files_to_list(retrieval_audio_files)
        self.retrieval_audio_files = [Path(retrieval_audio_files).parent / x for x in self.retrieval_audio_files]
    ...
    def __getitem__(self, index):
        ...
        retrieval_filename = self.retrieval_audio_files[index]
        retrieval_audio,sampling_rate = self.load_wav_to_torch(retrieval_filename)
        # Take segment
        ...
        if retrieval_audio.size(0) >= self.segment_length:
            max_audio_start = retrieval_audio.size(0) - self.segment_length
            audio_start = random.randint(0, max_audio_start)
            retrieval_audio = retrieval_audio[audio_start : audio_start + self.segment_length]
        else:
            retrieval_audio = F.pad(retrieval_audio, (0, self.segment_length - audio.size(0)), "constant").data
        # Read video
        video_filename = self.video_files[index]
        filename = os.path.basename(video_filename).split(".")[0]
        # print(filename)
        ...
        if self.use_genre:
            ...
            data = {'refer_music':retrieval_audio.unsqueeze(0),
