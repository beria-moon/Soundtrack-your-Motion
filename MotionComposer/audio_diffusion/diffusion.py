...
class ADPM2Sampler(Sampler):
    ...
    def step(self, x: Tensor, fn: Callable, sigma: float, sigma_next: float) -> Tensor:
        ...
        # Derivative at sigma (∂x/∂sigma)
        #print('debug sigma',x.shape,fn(x, sigma=sigma).shape)
        d = (x - fn(x, sigma=sigma)) / sigma
        # Denoise to midpoint
...
class Diffusion(nn.Module):
    ...
    def denoise_fn(
        ...
        **kwargs,
        ...
        x_denoised = c_skip * x_noisy + c_out * x_pred  #retrieval
    ...
    def forward(self, x: Tensor, noise: Tensor = None, **kwargs) -> Tensor:
        ...
        if batch==4:
            ...
            sigmas = self.sigma_distribution(num_samples=batch//2, device=device)  #retrieval batch//2
            sigmas_padded = rearrange(sigmas, "b -> b 1 1")

            # Add noise to input
            noise = default(noise, lambda: torch.randn_like(x))
            x_noisy = x + sigmas_padded * noise
            x_noisy = x + refer  #retrieval
            # Compute denoised values
            # print('debug train sigmas',sigmas.shape)
            x_denoised = self.denoise_fn(x_noisy, sigmas=sigmas, **kwargs)
            # print("debug",x_denoised.shape,x.shape)
            # audio_gen = x_denoised[0].transpose(0, 1).squeeze().detach().cpu().numpy()
            # print(audio_gen.shape)
            # sf.write('/mnt/sdb/wangjinting/A-V/LORIS/generated_1.wav', audio_gen, samplerate=22050)
            # exit()
            ...
            Loudloss = TFLoudnessRatio()
            loudloss = Loudloss(x_denoised,x)
            loss = loss + loudloss
        else:
            # Sample amount of noise to add for each batch element
            sigmas = self.sigma_distribution(num_samples=batch, device=device)  #retrieval batch//2
            sigmas_padded = rearrange(sigmas, "b -> b 1 1")
            ...
            x_denoised = self.denoise_fn(x_noisy, sigmas=sigmas, **kwargs)
            # print("debug",x_denoised.shape,x.shape)
            # audio_gen = x[0].transpose(0, 1).squeeze().detach().cpu().numpy()
            # print(audio_gen.shape)
            # sf.write('/mnt/sdb/wangjinting/A-V/LORIS/generated_1.wav', audio_gen, samplerate=22050)
...
class DiffusionSampler(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.sigma_distribution = LogNormalDistribution(mean=-3.0, std=1.0)
    ...
    def forward(
        ...
        if batch==4:
            ...
            sigmas = self.sigma_distribution(batch//2, device=device) #retrieval batch//2

            noise = noise+refer
            

            #sigmas = self.sigma_schedule(num_steps, device)
            # print('debug sigmas',sigmas.shape)

            x_denoised = self.denoise_fn(noise, sigmas=sigmas, **kwargs)
            # print("debug",x_denoised.shape)
            # audio_gen = x_denoised[0].transpose(0, 1).squeeze().detach().cpu().numpy()
            # print(audio_gen.shape)
            # sf.write('/mnt/sdb/wangjinting/A-V/LORIS/test_generated_1.wav', audio_gen, samplerate=22050)
            # exit()
            ...
            # print('deug sampler',self.sampler)
            #x = self.sampler(noise, fn=fn, sigmas=sigmas, num_steps=num_steps)
            #x_denoised = x.clamp(-1.0, 1.0)
        # print("debug",x_denoised.shape)
        # audio_gen = x_denoised[0].transpose(0, 1).squeeze().detach().cpu().numpy()
        # print(audio_gen.shape)
        # sf.write('/mnt/sdb/wangjinting/A-V/LORIS/test_sampler_generated_1.wav', audio_gen, samplerate=22050)
        # exit()
      
        else:
            '''
            sigmas = self.sigma_distribution(batch, device=device)
            x_denoised = self.denoise_fn(noise, sigmas=sigmas, **kwargs)
            '''
        ...
        # print('debug output shape',x_denoised.shape)
        return x_denoised
