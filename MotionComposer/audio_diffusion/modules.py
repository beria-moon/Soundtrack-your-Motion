...
import math
from .utils import default, exists, prod, normalize
...
class FiLM(nn.Module):
    def __init__(self, context_dim, condition_dim):
        super().__init__()
        self.context_dim = context_dim
        self.condition_dim = condition_dim
        self.generator_f = nn.Linear(condition_dim, context_dim)
        self.generator_h = nn.Linear(condition_dim, context_dim)
        # self.pooler = nn.Linear(50, 1)

    def forward(self, x, condition_feature):
        '''
        input x: bs, 4096(tem_len), 512(context_dim), condition_feature: bs, 50, 1024(condition_dim)
        '''
        condition_feature = torch.mean(condition_feature, dim=1, keepdim=True)

        condition_gamma = self.generator_f(condition_feature)
        condition_beta = self.generator_h(condition_feature)
        out = x * condition_gamma + condition_beta
        return out


class RhythmHawkesTransformer(nn.Module):
    def __init__(self, condition_dim, num_heads, head_features, context_rhythm_max_length):
        super().__init__()
        self.condition_dim = condition_dim
        self.embedding = nn.Embedding(2, condition_dim)
        self.pos_emb = HawkesPositionalEmbedding(condition_dim, context_rhythm_max_length)
        self.transformer = HawkesTransformerBlock1d(
                channels=condition_dim,
                num_heads=num_heads,
                head_features=head_features,
            )

    def forward(self, x, rhy_mask=None):
        rhy_emb = self.embedding(x.squeeze(2)) * math.sqrt(self.condition_dim)
        pos_emb = self.pos_emb(x)
        rhy_hawkes = self.transformer(rhy_emb+pos_emb)
        return rhy_hawkes


class HawkesTransformerBlock1d(nn.Module):
    def __init__(
        self,
        channels: int,
        *,
        ...
        super().__init__()
        self.attention = Attention(features=channels, num_heads=num_heads, head_features=head_features)
        self.feed_forward = EinopsToAndFrom(
            "b l c",
            "b c l",
            FeedForward1d(channels=channels, multiplier=multiplier)
        )
...
class HawkesPositionalEmbedding(nn.Module):
    ...
    def forward(self, x):
        interval = torch.ones(x.size(), device=x.device)
        phi = self.Wt(interval)
        arc = (self.position[:x.size(1)] * self.div_term).unsqueeze(0)
...
class ResnetBlock1d(nn.Module):
    def __init__(
        ...
        *,
        ...
        context_rhythm_max_length: Optional[int] = None,
        context_genre_features: Optional[int] = None,
    ) -> None:
        super().__init__()

        self.use_mapping = exists(context_mapping_features)
        self.use_embedding = exists(context_embedding_features)
        self.use_genre = exists(context_genre_features)
        ...
        if self.use_embedding:
            ...
            self.rhythm_hawkes_attention = RhythmHawkesTransformer(
                condition_dim=context_embedding_features,
                head_features=context_head_features,
                num_heads=context_heads,
                context_rhythm_max_length=context_rhythm_max_length,
            )
    ...
    def forward(
        ...
        assert_message = "context genre required if context_genre_features > 0"
        assert not (self.use_genre ^ exists(genre)), assert_message
        h = self.block1(x)
        if self.use_embedding:
            h = self.cross_attend(h, context=embedding) + h
            rhythm = rhythm.long()
            rhythm_hawkes = self.rhythm_hawkes_attention(rhythm)
            h = self.cross_attend(h, context=rhythm_hawkes) + h
            if self.use_genre:
                h = self.cross_attend(h, context=genre) + h
...
class AttentionBase(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        #print('debug einsum', q.shape, k.shape)
...
class DownsampleBlock1d(nn.Module):
        ...
        self.blocks = nn.ModuleList(
            [
                ResnetBlock1d(
                    ...
                    context_genre_features=context_genre_features,
                )
    ...
    def forward(
        ...
        for block in self.blocks:
            x = block(x, mapping=mapping, embedding=embedding, genre=genre, rhythm=rhythm)
...
class BottleneckBlock1d(nn.Module):
    ...
    def forward(
        ...
        x = self.pre_block(x, mapping=mapping, embedding=embedding, genre=genre, rhythm=rhythm)
        if self.use_attention:
            x = self.attention(x)
        x = self.post_block(x, mapping=mapping, embedding=embedding, genre=genre, rhythm=rhythm)
        return x
...
class UNet1d(nn.Module):
    def __init__(
        ...
        super().__init__()
        out_channels = default(out_channels, in_channels)  #retrieval
        context_channels = list(default(context_channels, []))
        num_layers = len(multipliers) - 1
        use_context_features = exists(context_features)
        use_context_channels = len(context_channels) > 0
        context_mapping_features = None
        self.context_rhythm_max_length = context_rhythm_max_length
    ...
    def forward(
        ...
        *,
        ...
        for i, downsample in enumerate(self.downsamples):
            ...
            x, skips = downsample(
                x, mapping=mapping, channels=channels, embedding=embedding, genre=genre, rhythm=rhythm
            )
            skips_list += [skips]
        # print(f"downsample x shape: {x.shape}")
        x = self.bottleneck(x, mapping=mapping, embedding=embedding, genre=genre, rhythm=rhythm)
        # print(f"bottleneck x shape: {x.shape}")
        for i, upsample in enumerate(self.upsamples):
            skips = skips_list.pop()
            x = upsample(x, skips, mapping=mapping, embedding=embedding, genre=genre, rhythm=rhythm)
...
class UNetConditional1d(UNet1d):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.fixed_rhythm = FixedEmbedding(
            context_rhythm_max_length, 1
        )
        self.fixed_genre = FixedEmbedding(
            1, context_embedding_features
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        # print(f"embedding shape: {embedding.shape}, fixed_embedding shape: {fixed_embedding.shape}")
        fixed_rhythm = self.fixed_rhythm(rhythm) if rhythm is not None else None
        #print(f"rhythm shape: {rhythm.shape}, fixed_rhythm shape: {fixed_rhythm.shape}",fixed_rhythm.max())
        fixed_rhythm = normalize(fixed_rhythm)
        
        fixed_genre = self.fixed_genre(genre) if genre is not None else None
        ...
        if embedding_mask_proba > 0.0:
            ...
            if rhythm is not None:
                rhythm = torch.where(batch_mask, fixed_rhythm, rhythm)
            if genre is not None:
                genre = torch.where(batch_mask, fixed_genre, genre)
        if embedding_scale != 1.0:
            # Compute both normal and fixed embedding outputs
            out = super().forward(x, time, embedding=embedding, rhythm=rhythm, genre=genre, **kwargs)
            out_masked = super().forward(x, time, embedding=fixed_embedding, rhythm=fixed_rhythm, genre=fixed_genre, **kwargs)
            # Scale conditional output using classifier-free guidance
            return out_masked + (out - out_masked) * embedding_scale
        else:
            return super().forward(x, time, embedding=embedding, rhythm=rhythm, genre=genre, **kwargs)
