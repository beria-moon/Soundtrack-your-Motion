...
class TFLoudnessRatio(nn.Module):
    ...
    def forward(self, out_sig: torch.Tensor, ref_sig: torch.Tensor) -> torch.Tensor:
        ...
        bands_ref = self.filter(ref_sig).view(B * self.n_bands, 2, -1)
        bands_out = self.filter(out_sig).view(B * self.n_bands, 2, -1)
        frame = int(self.segment * self.sample_rate)
        stride = int(frame * (1 - self.overlap))
        gt = _unfold(bands_ref, frame, stride).squeeze(1).contiguous().view(-1, 2, frame)
        est = _unfold(bands_out, frame, stride).squeeze(1).contiguous().view(-1, 2, frame)
