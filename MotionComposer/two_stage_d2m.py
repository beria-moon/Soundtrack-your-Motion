...
from d2m.loris_two import LORIS
...
def train(config, args, logger):
    ...
    if config['model']['use_pretrain'] is True:
        if autoencoder_type == 'autoencoder':
            ...
            # loris.cond_diffusion = torch.load()
            loris.cond_diffusion.load_state_dict(autoencoder_state_dict, strict=False)
            loris.refineNet.load_state_dict(autoencoder_state_dict,strict=False)
        else:
    ...
    for k, v in loris.refineNet.named_parameters():
         if k in autoencoder_state_dict.keys() and autoencoder_state_dict[k].numel() == v.numel():
             b_params.append(v)
         else:
             a_params.append(v)
    ...
    #### creat data loader ####
    train_dataset = S25Dataset(audio_files=config['audio_train_path'],retrieval_audio_files=config['audio_train_retrieval_path'], video_files=config['video_train_path'], motion_files=config['motion_train_path'], genre_label=config['genre_train_path'], augment=True, config=config['model'])
    val_dataset = S25Dataset(audio_files=config['audio_test_path'],retrieval_audio_files=config['audio_test_retrieval_path'], video_files=config['video_test_path'], motion_files=config['motion_test_path'], genre_label=config['genre_test_path'], augment=False, config=config['model'])
    ...
    for epoch in range(1, config['max_epochs']+1):
        ...
        for batch_idx, input in enumerate(train_loader):
            ...
            input['music'], input['refer_music'],input['motion'], input['video'],input['genre'] = input['music'].cuda(),input['refer_music'].cuda(), input['motion'].cuda(), input['video'].cuda(),input['genre'].cuda()
            step_num += 1
            sys.stdout.flush()
            opt.zero_grad()
            loss = loris(input)
        ...
        if epoch % config['log_interval'] == 0:
            print('Train Epoch: {}\tLoss: {:.6f}\tEpoch Time: {:.1f}s\tETA: {:2d}h{:2d}min'.format(
...
def main():
    ...
    config_path = os.path.join(args.config_path,"retrieval_dance_s25.yaml")
